...
以下是完整的真实数据训练预测流程，系统已全面移除Mock fallback，所有功能均依赖真实数据：
...
### 3. 上传训练数据
1. 进入「数据管理」页面
2. 点击「上传训练CSV」按钮，选择符合字段要求的训练数据文件
3. 上传成功后文件会保存到后端 `data/uploads/` 目录，数据集列表中会显示新上传的数据集

### 4. 数据质量检查与修复
1. 在数据集列表点击对应数据集的「质检」按钮
2. 系统会自动检查必填字段是否存在、日期缺失、数值负数、日期重复、lag字段缺失、数据行数是否足够等问题
3. 针对可修复问题（重复日期、缺失lag字段、检修/节假日空值），可以点击对应「处理」按钮单项修复，或点击「修复数据」按钮一键修复所有可修复问题
4. 修复后的数据会保存到 `data/processed/` 目录

### 5. 激活训练数据集
1. 在数据集列表操作栏点击「设为训练数据」
2. 该数据集会被设置为当前激活的训练数据集，后续训练任务会默认使用

### 6. 创建训练任务
1. 进入「训练任务」页面，点击「新建训练任务」
2. 选择模型类型、算法、训练数据范围，在「训练数据集」下拉框选择刚才激活的数据集
3. 点击「提交任务」开始训练，训练完成后会自动注册为 `candidate` 状态的模型
4. 训练过程可以查看实时日志，训练完成后可以查看评估指标

### 7. 发布生产模型
1. 进入「模型管理」页面，找到刚才训练生成的 `candidate` 模型
2. 点击「查看详情」确认模型指标符合要求
3. 点击「发布」按钮将模型设置为 `production` 状态，系统预测将自动使用该版本

### 8. 上传并激活预测数据
1. 回到「数据管理」页面，点击「上传预测输入CSV」，上传待预测数据文件
2. 上传成功后点击「设为预测输入」将其激活为当前预测数据集

### 9. 执行批量预测
1. 进入「预测监控」页面
2. 确认已激活预测数据集，点击「执行批量预测」按钮
3. 预测完成后会生成预测结果，可查看预测记录和误差指标

### 10. 查看预测结果与评估
1. 进入「模型评估」页面，查看预测指标、误差分布、特征重要性等评估结果
2. 预测结果文件保存到 `outputs/reports/` 目录
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/datasets/upload` | POST | 上传数据集文件，支持训练/预测两种类型 |
| `/api/datasets/{datasetId}/quality-check` | POST | 对指定数据集执行质量检查 |
| `/api/datasets/{datasetId}/repair` | POST | 修复数据集可修复问题 |
| `/api/datasets/{datasetId}/activate` | POST | 将数据集设置为当前激活的训练/预测数据集 |
| `/api/datasets` | GET | 获取所有数据集列表 |
| `/api/datasets/active` | GET | 获取当前激活的训练和预测数据集 |

> ⚠️ **注意**：现在 `/api/training/run` 和 `/api/predictions/run` 接口必须传入 `datasetId` 或存在已激活的对应类型数据集，否则会返回400错误，不再使用默认样例数据。
